...
const calc = (size, material, options, promocode, result) => {
  ...
  const calcFunc = () => {
    ...
    if (sizeBlock.value == "" || materialBlock.value == "" || !+materialBlock.value) {
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener("click", function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_1__["getResource"])("assets/db.json").then(res => createCatds(res.styles)).catch(error => console.log(error));
    this.remove();
  });

  function createCatds(response) {
    response.forEach(({
      src,
      title,
      link
